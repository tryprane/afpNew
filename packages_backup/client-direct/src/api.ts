...
import { portalManager } from "@elizaos-plugins/plugin-lensNetwork"
...
export function createApiRouter(
    ...
    router.post("/initilizethebrowser", async (req, res) => {
        try {
            ...
            if(portalManager.isPortalReady()){
               await portalManager.closePortal();
            }

            // if (portalManager.isPortalReady()) {
            //     return res.json({
            //         success: true,
            //         message: "Browser is already initialized and logged in"
            //     });
            // }

            const success = await portalManager.initializePortal(registrationNumber, password);
            if (!success) {
                return res.status(500).json({
                    error: "Failed to initialize browser or login failed"
                });
            }

            res.json({
                success: true,
                message: "Browser initialized successfully and successfully logged in"
            });
        } catch (error) {
            elizaLogger.error("Error initializing browser:", error);
            res.status(500).json({
